...
pub use atelier_importer::type_uuid::{self, TypeUuid};
...
impl Default for AssetServer {
    fn default() -> Self {
        ...
        AssetServer {
            ...
            loader: RpcLoader::new_with_handle_allocator(
                atelier_loader::rpc_loader::default_connect_string().to_string(),
...
impl AssetServer {
    ...
    pub fn process_system(_world: &mut bevy_ecs::World, resources: &mut Resources) {
        ...
        asset_server.loader.process(&resolver);
    }
}

struct AssetStorageResolver<'a, 'b>(&'a AssetTypeRegistry, &'b Resources);

impl<'a, 'b> atelier_loader::AssetStorage for AssetStorageResolver<'a, 'b> {
    fn update_asset(
        &self,
        loader_info: &dyn atelier_loader::LoaderInfoProvider,
        asset_type_id: &atelier_core::AssetTypeId,
        data: &[u8],
        load_handle: atelier_loader::LoadHandle,
        load_op: atelier_loader::AssetLoadOp,
        version: u32,
        ...
        if let Some(registration) = self.0.registrations.get(asset_type_id) {
            ...
            (registration.get_assets_storage_fn)(
                self.1,
                &mut |storage: &dyn atelier_loader::AssetStorage| {
    ...
    fn free(
        ...
        if let Some(registration) = self.0.registrations.get(asset_type_id) {
            (registration.get_assets_storage_fn)(
                self.1,
                &mut |storage: &dyn atelier_loader::AssetStorage| {
                    storage.free(asset_type_id, load_handle);
                },
            );
        } else {
            log::error!(
